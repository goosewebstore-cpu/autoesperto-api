...
export default function GuideTableOfContents({ items }: Props) {
  ...
  function scrollToSection(e: React.MouseEvent<HTMLAnchorElement>, id: string) {
    ...
    if (!el) {
      // Try searching by partial id or attribute
      el = document.querySelector(`[id*="${id}"]`) || document.querySelector(`section#${id}`);
    }
    ...
    // If element still not found, do NOT call preventDefault(), let default anchor jump handle it
  }
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-3.5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
              {items.length} sezioni da consultare
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className="px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200 dark:bg-slate-800 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold flex items-center gap-1 sm:hidden transition-colors cursor-pointer"
        >
