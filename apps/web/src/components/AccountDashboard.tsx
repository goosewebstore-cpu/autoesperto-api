      ...
      <main className="mx-auto max-w-6xl px-5 py-8 sm:py-12">
        ...
        {userTier !== 'premium' && (
          <section className="mt-8 grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,.07)] lg:grid-cols-[1.15fr_.85fr]">
            <div className="p-6 sm:p-9"><span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-extrabold text-blue-700"><LockKeyhole className="h-3.5 w-3.5" /> Sblocca Premium</span><h2 className="mt-5 text-3xl font-extrabold tracking-tight text-slate-950">Ottieni il massimo da AutoEsperto.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">L'analisi base (marca, modello, anno, colore e tipologia) è sempre gratuita. Passa a Premium per le analisi complete illimitate: prezzi reali di mercato, difetti frequenti, costi di riparazione e molto altro.</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{['Analisi complete illimitate', 'Generatore annunci di vendita', 'Zero pubblicità', 'Annulli quando vuoi'].map((item) => <div key={item} className="flex items-center gap-2 text-sm font-bold text-slate-700"><CheckCircle2 className="h-4 w-4 text-emerald-600" />{item}</div>)}</div></div>
