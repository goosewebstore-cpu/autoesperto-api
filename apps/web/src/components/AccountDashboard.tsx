...
export default function AccountDashboard({ checkout, sessionId }: { checkout?: string; sessionId?: string }) {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        if (/accesso richiesto|sessione|401/i.test(err.message || '')) {
          clearAuthToken();
          router.replace('/accesso?next=/account');
          return;
        }
        if (active) setError(err.message || 'Non riesco a caricare la tua area personale.');
      } finally {
  ...
  const startCheckout = async () => {
    ...
    try {
      ...
      setError(err.message || 'Non riesco ad aprire il pagamento sicuro.');
      setPaying(false);
    }
  };
  ...
  const analyzeFile = async (file?: File) => {
    ...
    try {
      ...
      setError(err.message || 'Non riesco a completare l’analisi. Il credito non è stato utilizzato.');
    } finally {
      setAnalyzing(false);
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-5 py-8 sm:py-12">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div><Link href="/" className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-blue-700"><ArrowLeft className="h-3.5 w-3.5" /> Home</Link><p className="mt-4 text-xs font-extrabold uppercase tracking-[.14em] text-blue-600">Area personale</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-950">Ciao {user.name || 'da AutoEsperto'}</h1><p className="mt-2 text-sm text-slate-600">{user.email || user.phone} · Analisi utilizzate {user.entitlement.used}/{user.entitlement.included}</p></div>
          <div className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-xs font-extrabold ${analysis ? 'bg-emerald-100 text-emerald-700' : user.entitlement.paid ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700'}`}><span className="h-2 w-2 rounded-full bg-current" />{analysis ? 'Report salvato' : user.entitlement.paid ? 'Analisi disponibile' : 'Pagamento richiesto'}</div>
        </div>
        ...
        {!user.entitlement.paid && !analysis && (
          <section className="mt-8 grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,.07)] lg:grid-cols-[1.15fr_.85fr]">
            <div className="p-6 sm:p-9"><span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-extrabold text-blue-700"><FileText className="h-3.5 w-3.5" /> Acquisto singolo</span><h2 className="mt-5 text-3xl font-extrabold tracking-tight text-slate-950">Una decisione più informata, senza abbonamento.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">Carichi una foto, AutoEsperto riconosce il veicolo, valuta ciò che è visibile e crea un report completo su affidabilità, problemi noti, valore indicativo, costi e controlli da fare.</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{['Una analisi completa', 'Report salvato nell’account', 'PDF scaricabile', 'Nessun rinnovo automatico'].map((item) => <div key={item} className="flex items-center gap-2 text-sm font-bold text-slate-700"><CheckCircle2 className="h-4 w-4 text-emerald-600" />{item}</div>)}</div></div>
            <div className="flex flex-col justify-center bg-slate-950 p-6 text-white sm:p-9"><p className="text-xs font-extrabold uppercase tracking-[.14em] text-blue-300">Analisi completa</p><div className="mt-3 text-4xl font-extrabold">5,99 €</div><p className="mt-1 text-sm text-slate-400">Pagamento unico · una analisi per account</p><button onClick={startCheckout} disabled={paying} className="mt-7 flex h-12 items-center justify-center gap-2 rounded-xl bg-blue-600 text-sm font-extrabold text-white transition hover:bg-blue-500 disabled:opacity-60">{paying ? <><Loader2 className="h-4 w-4 animate-spin" /> Apro Stripe…</> : <><CreditCard className="h-4 w-4" /> Acquista in sicurezza</>}</button><p className="mt-4 flex items-center gap-2 text-xs text-slate-400"><LockKeyhole className="h-3.5 w-3.5" /> Pagamento gestito da Stripe. AutoEsperto non vede i dati della carta.</p></div>
          </section>
        )}

        {user.entitlement.paid && !analysis && (
          <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,.07)] sm:p-9">
            <div className="mx-auto max-w-2xl text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-blue-50 text-blue-700"><Camera className="h-6 w-6" /></span><h2 className="mt-5 text-2xl font-extrabold tracking-tight text-slate-950">Crea la tua analisi</h2><p className="mt-2 text-sm leading-6 text-slate-600">Usa una foto nitida di tre quarti, con frontale o posteriore ben visibile. Se il veicolo non viene riconosciuto, l’analisi non viene consumata.</p><label className="mx-auto mt-5 flex max-w-xl cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-left text-xs leading-5 text-slate-600"><input type="checkbox" checked={immediateExecutionAccepted} onChange={(event) => setImmediateExecutionAccepted(event.target.checked)} className="mt-1 h-4 w-4 rounded border-slate-300 text-blue-600" /><span>Chiedo che l’analisi inizi subito e riconosco che, una volta generato interamente il report digitale, posso perdere il diritto di recesso nei limiti di legge. <Link href="/terms" className="font-bold text-blue-700 hover:underline">Leggi i Termini</Link>.</span></label><button onClick={() => inputRef.current?.click()} disabled={analyzing || !immediateExecutionAccepted} className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-6 text-sm font-extrabold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40">{analyzing ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisi dettagliata in corso…</> : <><Upload className="h-4 w-4" /> Carica la foto dell’auto</>}</button><input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => analyzeFile(event.target.files?.[0])} /><p className="mt-4 flex items-center justify-center gap-2 text-xs text-slate-500"><ShieldCheck className="h-3.5 w-3.5" /> Salviamo il report, non la fotografia originale.</p></div>
          </section>
        )}

        {analysis && (
          <div className="mt-8">
            <section className="rounded-3xl bg-slate-950 p-6 text-white sm:p-9"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-emerald-300">La tua analisi</p><h2 className="mt-3 text-3xl font-extrabold tracking-tight">{analysis.title}</h2><p className="mt-2 text-sm text-slate-400">Creata il {new Date(analysis.createdAt).toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })} · disponibile nel tuo account</p></div><div className="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-3 text-sm font-bold"><Check className="h-4 w-4 text-emerald-400" /> 1 di 1 utilizzata</div></div></section>
